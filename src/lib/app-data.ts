...
function buildTaskPhotoThumbnailUrl(kind: "completion" | "reference", photo: { id: string; storage_path: string; thumbnail_storage_path?: string | null }) {
  const basePath = kind === "completion" ? `/api/task-photos/${photo.id}` : `/api/task-reference-photos/${photo.id}`;
  const versionKey = photo.thumbnail_storage_path || photo.storage_path;
  return `${basePath}?thumb=1&v=${encodeURIComponent(versionKey)}`;
}
